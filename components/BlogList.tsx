...
const BlogList: React.FC = () => {
  ...
  const latestDate = posts[0]?.published_at
    ? new Date(posts[0].published_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })
    : '';
    ...
    <main className="min-h-screen bg-[#FAFAF8] font-sans">
      ...
      <div className="relative max-w-3xl mx-auto px-6">
        ...
        <header className="pt-32 pb-16 border-b border-slate-200/60">
          {/* Eyebrow */}
          <div className="flex items-center gap-2.5 mb-6 animate-fade-up" style={{ animationDelay: '0s' }}>
          ...
          {/* Title */}
          <h1
            className="font-serif text-[clamp(2.4rem,5.5vw,3.75rem)] leading-[1.08] text-slate-900 mb-5 animate-fade-up"
            style={{ animationDelay: '0.08s', opacity: 0 }}
          >
            Building with AI<span className="text-[#92680A]">.</span><br />
          ...
          >
            Running AI agent teams for a real business. Every win, every failure, every decision — documented as it happens.
          </p>
          ...
          >
            <span><span className="text-slate-600 font-medium">{postCount}</span> {postCount === 1 ? 'post' : 'posts'}</span>
        ...
        <section className="py-12">
          {loading ? (
            ...
            <p className="text-slate-400 font-mono text-sm">No posts yet. First one is in the pipeline.</p>
          ) : (
            <div className="divide-y divide-slate-200/60">
              {posts.map((post, index) => (
                <Link
                  ...
                  style={{ animationDelay: `${0.3 + index * 0.06}s`, opacity: 0 }}
                >
                  <div>
                    {/* Meta */}
                    ...
                    {/* Title */}
                    <h2 className="font-serif text-[clamp(1.15rem,2.5vw,1.5rem)] text-slate-900 group-hover:text-[#5C7080] transition-colors duration-200 mb-2">
                      {post.title}
                    </h2>
        ...
        <footer className="py-8 border-t border-slate-200/60">
          <div className="flex items-center justify-between">
            <Link to="/" className="font-mono text-[0.72rem] text-slate-400 hover:text-slate-900 transition-colors">
